...
{
    /*
    The Skeleton script sets the behaviour of a ground enemy game object.
    Apart from referencing other componenets, the max speed and movement values can be
    set within the Unity Inspector

    In game behaviour is walking along a straight parh by turning if a cliff or wall is detected.
    The Enemy Game Object will also attack if a Player is detected, while also utilising specific
    animations for individual states.
    */
    ...
    //Designate the cliff detection zone.
    ...
    //Check the Attack Cooldown value in the animation parameters and set it to the appropriate value after the Game Object has used an attack.
    ...
    void FixedUpdate()
    {
        ...
        if (!damageable.LockVelocity)
        {
            if (CanMove && touchingDirections.IsGrounded)
                //Character will start to accellerate towards max Speed, the clamp value limits the speed depending on facing direction
